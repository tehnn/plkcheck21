...
<html>
    <head>        
        <meta charset="utf-8">
        <title>21Files (PCU to PROV) CHECK</title>
        <style>
            .header{
                color: #fff5f8;
                font-size: 30px;
    ...
    <body>        
        <?php
        $mon = $_GET[mon];
        $sql = "select * from tehn_count21 where MON='$mon' order by pcucode ASC";
        ?>
        <div class="header">21Files (PCU to PROV) CHECK</div>
            ...
            <div style="margin-top: 10px; margin-left: 5px;">
			 <span><button class="btn btn-success" type="button" onclick="indx()">ย้อนกลับ</button></span>
                ...
                <select name="mon" id="mon" data-theme="d" onchange="go(this.value);">                                                    
                    <option value='2013-08' <?php
                    if ($mon == '2013-08') {
                        echo "selected";
                    }
                    ?>>สิงหาคม 2556</option>
                    <option value='2013-09' <?php
                    if ($mon == '2013-09') {
                        echo "selected";
                    }
                    ?>>กันยายน 2556</option>
                    <option value='2013-10' <?php
                    if ($mon == '2013-10') {
                        echo "selected";
                    }
                    ?>>ตุลาคม 2556</option>
                    <option value='2013-11' <?php
                    if ($mon == '2013-11') {
                        echo "selected";
                    }
                    ?>>พฤศจิกายน 2556</option>
                    <option value='2013-12' <?php
                    if ($mon == '2013-12') {
                        echo "selected";
                    }
                    ?>>ธันวาคม 2556</option>
                    <option value='2014-01' <?php
                    if ($mon == '2014-01') {
                        echo "selected";
                    }
                    ?>>มกราคม 2557</option>
                    <option value='2014-02' <?php
                    if ($mon == '2014-02') {
                        echo "selected";
                    }
        ...
        <div class="table-contain">
           <?php
			$sql_h = "SHOW FIELDS FROM tehn_count21";
			$sql_r = $sql;
